...
каких-либо дополнительных преобразований над ними.
2. Включать в записи логов расширенные (по сравнению с т.н. _mapped diagnostic context_, предлагаемым фреймворками логирования) 
...
#### Содержание
1. [Интеграция с фреймворками логирования](#Интеграция-с-фреймворками-логирования)
...
1. [log4j 2.8+](./logtrace-log4j2/Readme.md)
2. [logback-classic 1.2.*](./logtrace-logback/Readme.md)
3. [java.util.logging](./logtrace-jul/Readme.md)
4. [log4j 1.2.* (не рекомендуется, только для случаев когда миграция на log4j2 не возможна)](./logtrace-log4j/Readme.md)
...
В каталоге `./etc` приведены скрипты и конфигурационные файлы для сервисов, отвечающих за обработку и отправку логов в [elasticsearch](https://www.elastic.co/start):
1. [filebeat](https://www.elastic.co/downloads/beats/filebeat)  
Для запуска соответствующих docker контейнеров `elasticsearch`, `kibana`, `filebeat` требуется выполнить команду:  
`docker-compose -f ./docker-compose.yml up -d filebeat`
2. [fluent-bit](https://fluentbit.io/)  
Для запуска соответствующих docker контейнеров `elasticsearch`, `kibana`, `fluentbit` требуется выполнить команду:  
`docker-compose -f ./docker-compose.yml up -d fluentbit`
3. [vector](https://vector.dev)  
Для запуска соответствующих docker контейнеров `elasticsearch`, `kibana`, `vector` требуется выполнить команду:  
`docker-compose -f ./docker-compose.yml up -d vector`
...
Все логи будут отправлены в индексы: 
1. `logs-logtrace-*` (filebeat)
2. `logs-fluent-*` (fluentbit)
3. `logs-vector-*` (vector)
