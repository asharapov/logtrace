...
```
Здесь в атрибуте `thrown.hash` содержится MD5 хэш от стека ошибки, содержащейся в поле `thrown.stack`. Данный хэш удобно использовать для обработки дублирующихся ошибок.  
...
### История применения библиотеки

Несмотря на то что полноценного нагрузочного тестирования данного решения еще не проводилось,
имеющийся опыт его применения уже может дать некоторые цифры для первичной оценки:

|                   метрика                      |   значение        |
|------------------------------------------------|-------------------|
|Кол-во узлов ElasticSearch                      |                 1 |
|Выделено RAM на узел ElasticSearch, Gb          |                 8 |
|Ежедневное кол-во новых записей (в среднем)     | ~ 51 млн. записей |
|Частота индексации новых записей (в среднем)    | ~ 590 записей/сек |
|Кол-во записей всего                            |          646 млн. |
|Объем данных всего, Gb                          |             317.8 |
|Размер одной записи (в среднем), байт           |               528 |
